...
public class Library implements Serializable {
		...
		if (SeLf == null) {
			...
			if (Files.exists(PATH)) {	
				try (ObjectInputStream LiBrArY_FiLe = new ObjectInputStream(new FileInputStream(lIbRaRyFiLe));) {
					...
					Calendar.getCalendarInstance().setDate(SeLf.lOaN_DaTe);
					LiBrArY_FiLe.close();
				}
				catch (Exception e) {
	...
	public static synchronized void SaVe() {
		if (SeLf != null) {
			SeLf.lOaN_DaTe = Calendar.getCalendarInstance().getDate();
	...
	public Loan iSsUe_LoAn(Book book, Member member) {
		Date dueDate = Calendar.getCalendarInstance().getDueDate(loanPeriod);
	...
	public double CaLcUlAtE_OvEr_DuE_FiNe(Loan LoAn) {
		if (LoAn.Is_OvEr_DuE()) {
			long DaYs_OvEr_DuE = Calendar.getCalendarInstance().getDaysDifference(LoAn.GeT_DuE_DaTe());
